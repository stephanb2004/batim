...
public class StrikerEntity extends Monster implements GeoEntity {
	...
	public StrikerEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(BendymodModEntities.STRIKER.get(), world);
	}
	...
	private PlayState movementPredicate(AnimationState event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F))
				...
				return event.setAndContinue(RawAnimation.begin().thenLoop("animation.striker.walk"));
			}
			return event.setAndContinue(RawAnimation.begin().thenLoop("animation.striker.idle"));
		}
	...
	private PlayState attackingPredicate(AnimationState event) {
		...
		if (this.swinging && event.getController().getAnimationState() == AnimationController.State.STOPPED) {
			...
			return event.setAndContinue(RawAnimation.begin().thenPlay("animation.striker.attack"));
		}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(StrikerEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
