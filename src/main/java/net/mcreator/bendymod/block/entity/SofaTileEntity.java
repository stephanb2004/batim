...
import net.mcreator.bendymod.block.SofaBlock;

import javax.annotation.Nullable;

import java.util.stream.IntStream;

public class SofaTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.<ItemStack>withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());

	public SofaTileEntity(BlockPos pos, BlockState state) {
		super(BendymodModBlockEntities.SOFA.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		String animationprocedure = ("" + this.getBlockState().getValue(SofaBlock.ANIMATION));
	...
	public void registerControllers(AnimatableManager.ControllerRegistrar data) {
		data.add(new AnimationController<SofaTileEntity>(this, "controller", 0, this::predicate));
		data.add(new AnimationController<SofaTileEntity>(this, "procedurecontroller", 0, this::procedurePredicate));
	}
